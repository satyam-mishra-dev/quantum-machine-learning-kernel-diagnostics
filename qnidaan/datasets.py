...
CUMIDA_URL = (
    "https://sbcb.inf.ufrgs.br/data/cumida/Genes/Renal/GSE53757/"
    "Renal_GSE53757.csv"
)
...
def load_cumida():
    """CuMiDa Renal_GSE53757: 143 GEO microarray samples x 54,675 gene
    probes (Affymetrix HG-U133 Plus 2), clear cell renal cell carcinoma
    (n=71) vs normal kidney tissue (n=72). Already binary — no classes
    dropped. y: 1 = ccRCC (tumor), 0 = normal.

    Citation (CuMiDa asks for it): Feltes BC, Chandelier EB, Grisci BI,
    Dorn M. "CuMiDa: An Extensively Curated Microarray Database for
    Benchmarking and Testing of Machine Learning Approaches in Cancer
    Research." J Comput Biol. 2019;26(4):376-386. doi:10.1089/cmb.2018.0238
    """
    path = _fetch(CUMIDA_URL, "cumida_renal_gse53757.csv")
    df = pd.read_csv(path)
    y = (df.pop("type") == "ccRCC").astype(int).to_numpy()
    df = df.drop(columns=["samples"])  # GEO sample ids, not features
    return df.to_numpy(dtype=float), y, {
        "name": "cumida",
        "title": "CuMiDa Renal GSE53757 (ccRCC vs normal, 54,675 genes)",
        "n_features": df.shape[1],
        "feature_names": list(df.columns),
    }
